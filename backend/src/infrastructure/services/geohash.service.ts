...
function encodeGeohash(latitude: number, longitude: number, length: number): string {
    let isEven = true;
    let bit = 0;
    let ch = 0;
    let geohash = '';
...
export function computeGeohash(latitude: number, longitude: number, precisionMeters = 100): string {
