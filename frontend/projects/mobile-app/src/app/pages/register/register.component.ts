...
import { eye, eyeOff, arrowBackOutline, logoGoogle, logoFacebook } from 'ionicons/icons';
...
export class RegisterComponent {
    ...
    onSubmit(): void {
        ...
        this.http.post<any>(`${environment.apiUrl}/api/auth/register`, request).subscribe({
            next: (response) => {
    ...
    registerWithGoogle(): void {
        ...
        this.http.get<any>(`${environment.apiUrl}/api/auth/google`).subscribe({
            ...
            error: (error) => {
                this.isLoading.set(false);
                this.errorMessage.set(
                    error.error?.message || 'Error al conectar con Google. Intenta de nuevo.'
                );
            }
        });
    }
    ...
    registerWithFacebook(): void {
        ...
        this.http.get<any>(`${environment.apiUrl}/api/auth/facebook`).subscribe({
            ...
            error: (error) => {
                this.isLoading.set(false);
                this.errorMessage.set(
                    error.error?.message || 'Error al conectar con Facebook. Intenta de nuevo.'
                );
            }
        });
    }
